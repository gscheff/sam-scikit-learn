...
```bash
sam-scikit-learn$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-scikit-learn$ sam local start-api
sam-scikit-learn$ curl http://localhost:3000/classify_digit
```
...
```bash
sam-scikit-learn$ sam logs -n InferenceFunction --stack-name sam-scikit-learn --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-scikit-learn
```
